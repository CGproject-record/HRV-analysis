...
# load data 
mydata=read.csv("Sepsis3_rf_top10_cgmh_data_with_score.csv",header=TRUE)
...
availvar<-c("fFdP",
            ...
            "QSE",
            "IoV",
            "Mean.rate","mean","LF.Power.LombScargle")
...
mydata_new<-cbind(sofa3m=mydata$sofa3m,mydata[,availvar])
mydata_new$sofa3m<-factor(mydata_new$sofa3m,levels = c("Y","N"))
##
dim(mydata_new)
set.seed(123)
mydata1=mydata_new[which(mydata_new$sofa3m=="Y"),]
mydata2=mydata_new[which(mydata_new$sofa3m=="N"),]
...
nnet_train<-train(sofa3m~.,
                  data=training,
                  method="nnet",
                  tuneLength=20,
                  verbose=FALSE,
                  metric="ROC",
                  trControl=fitControl,MaxNWts=15000)



rp1_train<-train( sofa3m~.,
                  data=training,
                  method="rpart",
                  metric="ROC",
                  tuneLength=20,
...
set.seed(62)
rf_train<-train(sofa3m~.,
                data=training,
                method="rf",
                metric="ROC",
                tuneLength=10,ntree=20,
                trControl=fitControl)
...
xgbtra<-confusionMatrix(predict(xgb_train,training),training$sofa3m)$overall["Accuracy"]
xgbtea<-confusionMatrix(predict(xgb_train,testing),testing$sofa3m)$overall["Accuracy"]
train_results=predict(xgb_train,training,type="prob")
test_results=predict(xgb_train,testing,type="prob")
train_results$obs=training$sofa3m
train_results$pred=predictions_train
test_results$obs=testing$sofa3m
test_results$pred=predictions_test
xgb_trauc<-roc(training$sofa3m,train_results[,"Y"],levels=c("Y","N"))$auc
xgb_teauc<-roc(testing$sofa3m,test_results[,"Y"],levels=c("Y","N"))$auc
...
sdwdtra<-confusionMatrix(predict(sdwd_train,training),training$sofa3m)$overall["Accuracy"]
sdwdtea<-confusionMatrix(predict(sdwd_train,testing),testing$sofa3m)$overall["Accuracy"]
...
test_results$obs=testing$sofa3m
test_results$pred=predictions_test
sdwd_trauc<-roc(training$sofa3m,train_results[,"Y"],levels=c("Y","N"))$auc
sdwd_teauc<-roc(testing$sofa3m,test_results[,"Y"],levels=c("Y","N"))$auc
...
nnettra<-confusionMatrix(predict(nnet_train,training),training$sofa3m)$overall["Accuracy"]
nnettea<-confusionMatrix(predict(nnet_train,testing),testing$sofa3m)$overall["Accuracy"]
...
test_results$obs=testing$sofa3m
test_results$pred=predictions_test
nnet_trauc<-roc(training$sofa3m,train_results[,"Y"],levels=c("Y","N"))$auc
nnet_teauc<-roc(testing$sofa3m,test_results[,"Y"],levels=c("Y","N"))$auc
...
rp1tra<-confusionMatrix(predict(rp1_train,training),training$sofa3m)$overall["Accuracy"]
rp1tea<-confusionMatrix(predict(rp1_train,testing),testing$sofa3m)$overall["Accuracy"]
...
test_results$obs=testing$sofa3m
test_results$pred=predictions_test
rp1_trauc<-roc(training$sofa3m,train_results[,"Y"],levels=c("Y","N"))$auc
rp1_teauc<-roc(testing$sofa3m,test_results[,"Y"],levels=c("Y","N"))$auc
...
rp2tra<-confusionMatrix(predict(rp2_train,training),training$sofa3m)$overall["Accuracy"]
rp2tea<-confusionMatrix(predict(rp2_train,testing),testing$sofa3m)$overall["Accuracy"]
...
test_results$obs=testing$sofa3m
test_results$pred=predictions_test
rp2_trauc<-roc(training$sofa3m,train_results[,"Y"],levels=c("Y","N"))$auc
rp2_teauc<-roc(testing$sofa3m,test_results[,"Y"],levels=c("Y","N"))$auc
...
rftra<-confusionMatrix(predict(rf_train,training),training$sofa3m)$overall["Accuracy"]
rftea<-confusionMatrix(predict(rf_train,testing),testing$sofa3m)$overall["Accuracy"]
...
test_results$obs=testing$sofa3m
test_results$pred=predictions_test
rf_trauc<-roc(training$sofa3m,train_results[,"Y"],levels=c("Y","N"))$auc
rf_teauc<-roc(testing$sofa3m,test_results[,"Y"],levels=c("Y","N"))$auc
...
dnn_train<-read.csv('dnn for spesis3 in cgmh train result ver2 20230222.csv',header = T)
head(dnn_train)
...
dnn_test<-read.csv('dnn for spesis3 in cgmh test result ver2 20230222.csv',header = T)
head(dnn_test)
...
dnntra<-confusionMatrix(predictions_train,training$sofa3m)$overall["Accuracy"]
dnntea<-confusionMatrix(predictions_test,testing$sofa3m)$overall["Accuracy"]
dnn_trauc<-roc(training$sofa3m,dnn_train$prob_pred,levels=c("Y","N"))$auc
dnn_teauc<-roc(testing$sofa3m,dnn_test$prob_pred,levels=c("Y","N"))$auc
...
transfor_CI<-function( model ,data ) {
  tr_con<-confusionMatrix(predict(model,data),data$sofa3m)$table
  
  library(epiR)
  rval <- epi.tests(tr_con, conf.level = 0.95)
  
  # train
  AUC_result<-roc(data$sofa3m, predict(model,data,type='prob')[,"Y"],levels=c("Y","N"),CI=TRUE)
...
dnn_train$class_pred <-factor(ifelse(dnn_train$class_pred=="0","N","Y"),levels=c("Y","N"))
tr_con<-confusionMatrix(dnn_train$class_pred ,training$sofa3m)$table
library(epiR)
rval <- epi.tests(tr_con, conf.level = 0.95)
...
# train
AUC_result<-roc(training$sofa3m, dnn_train$prob_pred,levels=c("Y","N"),CI=TRUE)
as.numeric(AUC_result$auc)
ci_auc_result<-ci(AUC_result)
ci_auc_result<-as.numeric(ci_auc_result)
ci_auc_result
...
dnn_test$class_pred <-factor(ifelse(dnn_test$class_pred=="0","N","Y"),levels=c("Y","N"))
tr_con<-confusionMatrix(dnn_test$class_pred ,testing$sofa3m)$table
library(epiR)
rval <- epi.tests(tr_con, conf.level = 0.95)
...
# train
AUC_result<-roc(testing$sofa3m, dnn_test$prob_pred,levels=c("Y","N"),CI=TRUE)
as.numeric(AUC_result$auc)
ci_auc_result<-ci(AUC_result)
ci_auc_result<-as.numeric(ci_auc_result)
ci_auc_result
...
availvar<-c("fFdP",
            ...
            "QSE",
            "IoV",
            "Mean.rate","mean","LF.Power.LombScargle",'sofam','MEWSm','NEWSm')
...
tr_con<-confusionMatrix(factor(ifelse(delta_SOFA_train>cutoff,"Y","N"),levels=c('Y','N')),training$sofa3m)$table

library(epiR)
rval <- epi.tests(tr_con, conf.level = 0.95)

# train MEWS
AUC_result<-roc(training$sofa3m, delta_SOFA_train,levels=c("Y","N"),CI=TRUE)
as.numeric(AUC_result$auc)
ci_auc_result<-ci(AUC_result)
ci_auc_result<-as.numeric(ci_auc_result)
ci_auc_result
...
tr_con<-confusionMatrix(factor(ifelse(MEWS_train>cutoff,"Y","N"),levels=c('Y','N')),training$sofa3m)$table

library(epiR)
rval <- epi.tests(tr_con, conf.level = 0.95)

# train MEWS
AUC_result<-roc(training$sofa3m, MEWS_train,levels=c("Y","N"),CI=TRUE)
as.numeric(AUC_result$auc)
ci_auc_result<-ci(AUC_result)
ci_auc_result<-as.numeric(ci_auc_result)
ci_auc_result
...
tr_con<-confusionMatrix(factor(ifelse(NEWS_train>cutoff,"Y","N"),levels=c('Y','N')),training$sofa3m)$table

library(epiR)
rval <- epi.tests(tr_con, conf.level = 0.95)

# train MEWS
AUC_result<-roc(training$sofa3m, NEWS_train,levels=c("Y","N"),CI=TRUE)
as.numeric(AUC_result$auc)
ci_auc_result<-ci(AUC_result)
ci_auc_result<-as.numeric(ci_auc_result)
ci_auc_result
...
tr_con<-confusionMatrix(factor(ifelse(delta_SOFA_test>cutoff,"Y","N"),levels=c('Y','N')),testing$sofa3m)$table

library(epiR)
rval <- epi.tests(tr_con, conf.level = 0.95)

# train MEWS
AUC_result<-roc(testing$sofa3m, delta_SOFA_test,levels=c("Y","N"),CI=TRUE)
as.numeric(AUC_result$auc)
ci_auc_result<-ci(AUC_result)
ci_auc_result<-as.numeric(ci_auc_result)
ci_auc_result
...
tr_con<-confusionMatrix(factor(ifelse(MEWS_test>cutoff,"Y","N"),levels=c('Y','N')),testing$sofa3m)$table

library(epiR)
rval <- epi.tests(tr_con, conf.level = 0.95)

# train MEWS
AUC_result<-roc(testing$sofa3m, MEWS_test,levels=c("Y","N"),CI=TRUE)
as.numeric(AUC_result$auc)
ci_auc_result<-ci(AUC_result)
ci_auc_result<-as.numeric(ci_auc_result)
ci_auc_result
...
tr_con<-confusionMatrix(factor(ifelse(NEWS_test>cutoff,"Y","N"),levels=c('Y','N')),testing$sofa3m)$table

library(epiR)
rval <- epi.tests(tr_con, conf.level = 0.95)

# train MEWS
AUC_result<-roc(testing$sofa3m, NEWS_test,levels=c("Y","N"),CI=TRUE)
as.numeric(AUC_result$auc)
ci_auc_result<-ci(AUC_result)
ci_auc_result<-as.numeric(ci_auc_result)
ci_auc_result
...
training<-training[,c("sofa3m",availvar[1:10])]
...
testing<-testing[,c("sofa3m",availvar[1:10])]
#----------------------------------------------------------
...
xgboost_newTest_original<-as.data.frame(cbind(class = testing$sofa3m,xgboost_prob  =test_results[,"Y"] ))
...
rocobj2 <- lines.roc(roc(testing$sofa3m,deltaSOFA), percent=TRUE, col="green",lty = 2)
rocobj3 <- lines.roc(roc(testing$sofa3m,testing$fFdP), percent=TRUE, col="blue",lty = 3)
rocobj4 <- lines.roc(roc(testing$sofa3m,testing$aFdP),  percent=TRUE, col="darkgoldenrod",lty = 4)
rocobj5 <- lines.roc(roc(testing$sofa3m,testing$DFA.AUC), percent=TRUE, col="cyan",lty = 5)
rocobj6 <- lines.roc(roc(testing$sofa3m,NEWS), percent=TRUE, col="firebrick",lty = 6)
rocobj7 <- lines.roc(roc(testing$sofa3m,MEWS), percent=TRUE, col="darkorchid",lty = 7)




mod_roc<-round( roc(xgboost_newTest_original$class,xgboost_newTest_original$xgboost_prob)$auc,3)
var1_roc<-round(roc(testing$sofa3m,testing$fFdP)$auc,3)
var2_roc<-round(roc(testing$sofa3m,testing$aFdP)$auc,3)
var3_roc<-round(roc(testing$sofa3m,testing$DFA.AUC)$auc,3)
var4_roc<-round(roc(testing$sofa3m,deltaSOFA)$auc,3)
var5_roc<-round(roc(testing$sofa3m,MEWS)$auc,3)
var6_roc<-round(roc(testing$sofa3m,NEWS)$auc,3)


le0<-c("XGBoost","delta SOFA","fFdP","aFdP","DFA.AUC","NEWS","MEWS")
