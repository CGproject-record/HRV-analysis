...
# load data 
mydata<-read.csv('Siover1_rf_top10_mimic_data.csv',header = T)

table(mydata$si1m )
mydata$si1m <-ifelse(mydata$si1m=="1","Y","N")
...
# load data 
mydata=read.csv("Siover1_rf_top10_mimic_data.csv",header=TRUE)
mydata$si1m <-ifelse(mydata$si1m=="1","Y","N")
...
dnn_train<-read.csv('dnn for si over 1 in mimic train result ver2 20230223.csv',header = T)
head(dnn_train)
...
dnn_test<-read.csv('dnn for si over 1 in mimic test result ver2 20230223.csv',header = T)
head(dnn_test)
...
exter_valid<- read.csv('Siover1_rf_top10_cgmh_data_with_score.csv',header = T)
exter_valid$si1m<-factor(exter_valid$si1m,levels=c("Y","N"))
...
mydata_new<-cbind(si1m=mydata$si1m,mydata[,availvar])
mydata_new$si1m<-factor(ifelse(mydata_new$si1m=="1","Y","N"),levels = c("Y","N"))
##
dim(mydata_new)
set.seed(123)
mydata1=mydata_new[which(mydata_new$si1m=="Y"),]
...
mydata0=read.csv("Siover1_rf_top10_cgmh_data_with_score.csv",header=TRUE)

new_test<-mydata0[,names(training)]
new_test$si1m<-factor(new_test$si1m,levels = c("Y","N"))
table(new_test$si1m)
...
rocobj2 <- lines.roc(roc(testing$si1m,testing$Mean.rate), percent=TRUE, col="green",lty = 2)
rocobj3 <- lines.roc(roc(testing$si1m,testing$gcount), percent=TRUE, col="blue",lty = 3)
...
var1_roc<-round(roc(testing$si1m,testing$Mean.rate)$auc,3)
var2_roc<-round(roc(testing$si1m,testing$gcount)$auc,3)
var3_roc<-round(roc(testing$si1m,testing$sgridTAU)$auc,3)



le0<-c("XGBoost","Mean rate","gcount","sgridTAU")
roc0<-c(mod_roc,var1_roc,var2_roc,var3_roc)


roc_sort<-sort(roc0,decreasing = T)

legend("bottomright", legend=paste(le0," (AUC = ",roc_sort,")" ,sep=''),
       col=c("red","green","blue","darkgoldenrod"), lwd=1.2,cex = 1.1,lty = 1:7)
